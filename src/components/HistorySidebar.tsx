...
export function HistorySidebar({ history }: { history: number[] }) {
  ...
  if (!history || history.length === 0) {
    return (
      <div className="hidden lg:flex w-14 h-full rounded-2xl border border-white/10 bg-white/[0.03] backdrop-blur-sm items-center justify-center shrink-0">
        <span className="text-[10px] font-bold text-white/30 -rotate-90 whitespace-nowrap">No History</span>
      </div>
    );
  }
  ...
  return (
    <>
      <div
        ref={scrollRef}
        className="hidden lg:flex flex-col gap-2 overflow-y-auto items-center w-14 h-full py-3 px-1.5 rounded-2xl border border-white/10 bg-white/[0.03] backdrop-blur-sm no-scrollbar flex-shrink-0"
        style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}
      >
        {history.map((mult, idx) => {
          const animationClass = idx === 0 ? "animate-in slide-in-from-top-4 fade-in duration-300" : "";
          return (
            <button
              key={`${idx}-${mult}`}
              onClick={() => setSelected(mult)}
              className={`font-mono font-bold text-[11px] px-2 py-1.5 rounded-full border w-full text-center cursor-pointer hover:scale-105 transition-transform ${getColorClasses(mult)} ${animationClass}`}
            >
